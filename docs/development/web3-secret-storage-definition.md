...
To make your app work on DAC Blockchain, you can use the web3 object provided by the web3.js library. Under the hood it communicates to a local node through RPC calls. [web3](https://github.com/ChainSafe/web3.js) works with any DAC Node, which exposes an RPC layer.

web3 contains the eth object - web3.eth (for specifically DAC Blockchain interactions) and the shh object - web3.shh (for Whisper interaction). Over time we'll introduce other objects for each of the other web3 protocols. Working examples can be found here.
...
Secret key files are stored directly in `~/.dacnode/keystore` (for Unix-like systems) and `~/AppData/Dacnode/keystore` (for Windows). They may be named anything, but a good convention is `<uuid>.json`, where `<uuid>` is the 128-bit UUID given to the secret key (a privacy-preserving proxy for the secret key's address).
...
File contents of `~/.dacnode/keystore/3198bc9c-6672-5ab3-d9954942343ae5b6.json`:
